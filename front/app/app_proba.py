...
def test():
    ...
    prediction = pred(r)[0]
    """proba = pred(r)[1]"""
    #proba_neg = proba.replace('[','').replace(']','').split(' ')[0]
	#proba_pos = proba.replace('[','').replace(']','').split(' ')[1]
    pb_detect = None
    if prediction == "oups":
        pb_detect = "exist"
    else:
    	proba = pred(r)[1]
    return render_template('test.html', review=r, prediction=prediction, langue=TextBlob(r).detect_language(), pb_detect=pb_detect, proba = proba)
...
def analyse_2():
    ...
    #prediction = "positive"
    if l == "English":
    	prediction = pred_en(r)[0]
    elif l == "French":
    	prediction = pred_fr(r)[0]
...
def pred_fr(r):
    ...
    proba = str(model_fr.predict_proba(r_array)[0])
    pred  = model_fr.predict(r_array)[0]
    return class_review[pred], proba
...
def pred_en(r):
    ...
    r_array = [lt]
    """
    proba = str(model_en.predict_proba(r_array)[0])
