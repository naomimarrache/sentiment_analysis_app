...
def get_restau_reviews(url_restau) : 
    ...
    # Click the "english" radio button if exists 
    if (check_exists_by_xpath("//input[@id='filters_detail_language_filterLang_en']")):
        try : 
            print("Click 'english' radio button.... " )
            driver.find_element_by_xpath("//input[@id='filters_detail_language_filterLang_en']").click()
            time.sleep(3)
            
        except (ElementClickInterceptedException,StaleElementReferenceException) : 
            print("Intercept Click Exceptions when click '... radio button'" )
            # wait for the radio button to be Clickable 
            wait = WebDriverWait(driver, 60)
            wait.until(expected_conditions.element_to_be_clickable((By.XPATH, "//input[@id='filters_detail_language_filterLang_en']")))
            # Click the radio button as soon as it will be Clickable
            ActionChains(driver).move_to_element(driver.find_element_by_xpath("//input[@id='filters_detail_language_filterLang_en']")).click().perform()

    else : # There is no English reviews --> function return 
        print ("English Reviews not found !! ")
        return 
...
reviews_file =Path.cwd()/'all_restau_reviews_Eng.csv'
print(reviews_file)
